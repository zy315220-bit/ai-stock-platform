...
def backtest_stock(stock_code: str, start_date: str = "2023-01-01", end_date: str | None = None, entry_score: float = 75, exit_score: float = 55, initial_capital: float = 100_000, commission_rate: float = COMMISSION_RATE, transaction_tax_rate: float = ETF_TRANSACTION_TAX_RATE) -> dict[str, Any]:
    ...
    for index in range(first_trade_index, len(df) - 1):
        ...
        if shares == 0 and score >= entry_score:
            ...
            if purchasable_shares > 0:
                ...
                entry_price, entry_date, entry_signal_score = next_open, next_date, score; entry_gross_amount, entry_commission, entry_total_cost = buy_cost["gross_amount"], buy_cost["commission"], buy_cost["total_cost"]; total_commission += entry_commission
    ...
    final_close = float(df.iloc[-1]["Close"]); final_date = _get_row_date(df.iloc[-1]); final_equity = cash + shares * final_close
    ...
    _enrich_trades_with_excursions(trades=trades, df=trade_df)
    ...
    advanced_trade_statistics = _calculate_advanced_trade_statistics(trades=enriched_trades, initial_capital=normalized_capital, final_capital=final_equity, max_drawdown_percent=abs(max_drawdown), exposure_percent=exposure_percent)
