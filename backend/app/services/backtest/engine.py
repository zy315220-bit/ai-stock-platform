...
EXIT_MODES = {"score", "score_or_time", "score_or_ema_reversal", "score_or_time_or_ema_reversal"}
...
def backtest_stock(stock_code: str, start_date: str = "2023-01-01", end_date: str | None = None, entry_score: float = 75, exit_score: float = 55, initial_capital: float = 100_000, commission_rate: float = COMMISSION_RATE, transaction_tax_rate: float = ETF_TRANSACTION_TAX_RATE, require_ema_trend: bool = False, ema_fast_column: str = "EMA20", ema_slow_column: str = "EMA60", exit_mode: str = "score", max_holding_days: int = 60) -> dict[str, Any]:
    ...
    if exit_mode not in EXIT_MODES: raise ValueError(f"Unsupported exit_mode: {exit_mode}")
    if not isinstance(max_holding_days, int) or not 2 <= max_holding_days <= 252: raise ValueError("max_holding_days must be between 2 and 252 trading days.")
    ...
    df = _prepare_stock_data(df=df, start_date=warmup_start, end_date=end_date); df = add_indicators(df.copy())
    if df is None or df.empty: raise ValueError("技術指標計算後沒有可用資料。")
    df = df.dropna().reset_index(drop=True); df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
    ...
    cash, shares = normalized_capital, 0; entry_price = entry_date = entry_signal_score = None; entry_index: int | None = None
    ...
    for index in range(first_trade_index, len(df) - 1):
        historical_df = df.iloc[: index + 1].copy(); current_row, next_row = df.iloc[index], df.iloc[index + 1]
        score = _extract_score(calculate_score(historical_df)); signal_date, next_date = _get_row_date(current_row), _get_row_date(next_row); next_open = float(next_row["Open"])
        ema_trend_ok = (not require_ema_trend) or float(current_row[ema_fast_column]) > float(current_row[ema_slow_column])
        if shares == 0 and score >= entry_score and ema_trend_ok:
            ...
            if purchasable_shares > 0:
                buy_cost = _calculate_buy_cost(purchasable_shares, next_open, commission_rate); shares = purchasable_shares; cash -= buy_cost["total_cost"]
                entry_price, entry_date, entry_signal_score, entry_index = next_open, next_date, score, index + 1; entry_gross_amount, entry_commission, entry_total_cost = buy_cost["gross_amount"], buy_cost["commission"], buy_cost["total_cost"]; total_commission += entry_commission
        elif shares > 0:
            held_days = max(0, index - entry_index + 1) if entry_index is not None else 0
            score_exit = score <= exit_score
            time_exit = exit_mode in {"score_or_time", "score_or_time_or_ema_reversal"} and held_days >= max_holding_days
            ema_exit = exit_mode in {"score_or_ema_reversal", "score_or_time_or_ema_reversal"} and float(current_row[ema_fast_column]) <= float(current_row[ema_slow_column])
            if score_exit or time_exit or ema_exit:
                exit_reason = "score" if score_exit else "max_holding_days" if time_exit else "ema_reversal"
                sell_value = _calculate_sell_value(shares, next_open, commission_rate, transaction_tax_rate); cash += sell_value["net_amount"]; total_commission += sell_value["commission"]; total_transaction_tax += sell_value["transaction_tax"]
                trades.append({"entry_date": entry_date, "exit_date": next_date, "entry_price": entry_price, "exit_price": next_open, "shares": shares, "entry_signal_score": entry_signal_score, "exit_signal_score": score, "exit_reason": exit_reason, "holding_days": held_days, "entry_gross_amount": entry_gross_amount, "entry_commission": entry_commission, "entry_total_cost": entry_total_cost, "exit_gross_amount": sell_value["gross_amount"], "exit_commission": sell_value["commission"], "transaction_tax": sell_value["transaction_tax"], "exit_net_value": sell_value["net_amount"], "profit": sell_value["net_amount"] - entry_total_cost})
                shares = 0; entry_price = entry_date = entry_signal_score = entry_index = None; entry_gross_amount = entry_commission = entry_total_cost = 0.0
