...
def run_competition_on_frames(
    ...
    *,
    ...
    for spec in legacy.ROBOT_SPECS:
        ...
        for segment, start, end in (
            ...
            segment_outputs[segment]["open_positions"] = [
                position for result in symbol_results for position in result.get("open_positions", [])
            ]
    ...
    return {
        ...
        "periods": {
            "backtest": {"start": legacy._date_text(backtest_start), "end": legacy._date_text(backtest_end), "purpose": "固定規則的 2 個月歷史檢查，不在執行中調參。"},
            "forward": {"start": legacy._date_text(forward_start), "end": legacy._date_text(latest_date), "purpose": "最後 1 個月 walk-forward 模擬；正式排名只使用此區間。"},
        },
        "fairness": {
            ...
            "cost_model_id": cost_model.model_id,
            "instrument_type": cost_model.instrument_type,
            "commission_rate": cost_model.commission_rate,
            "transaction_tax_rate": cost_model.transaction_tax_rate,
        ...
        "leader": {
            "robot_id": leader["robot_id"], "name": leader["name"], "rank": 1, "qualified": qualified,
            "reason": "已達最低前瞻交易樣本門檻。" if qualified else f"目前僅 {forward_trades} 筆前瞻交易，未達 {legacy.MIN_FORWARD_TRADES_FOR_CHAMPION} 筆門檻。",
        },
