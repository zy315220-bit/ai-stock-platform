...
def frame_coverage(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    observed_months = {
        str(period)
        for period in pd.PeriodIndex(pd.to_datetime(frame.index), freq="M")
    }
    expected_months = {
        str(period)
    ...
    complete_month_coverage = not missing_months

    if not complete_month_coverage:
        coverage["status"] = "incomplete_months"
        coverage["long_horizon_qualified"] = False

    return {
        "start": start.strftime("%Y-%m-%d"),
        "end": end.strftime("%Y-%m-%d"),
        **coverage,
        "complete_month_coverage": complete_month_coverage,
