...
def _effective_trial_count(trial_return_paths: dict[str, Iterable[float]] | None, raw_count: int) -> dict[str, Any]:
    ...
    supplied = {
        name: cleaned_values
        for name, values in trial_return_paths.items()
        if len(cleaned_values := _clean(values)) >= 3
    }
    if len(supplied) < 2:
        return {
            "effective_trial_count": float(raw_count),
            "average_pairwise_correlation": None,
            "method": "raw_count_fallback",
            "fallback_used": True,
            "reason": "insufficient_trial_return_paths_for_correlation",
            "correlation_sample_count": len(supplied),
        }
    cleaned = _deduplicate_return_paths(supplied)
    if len(cleaned) < 2:
        return {
            "effective_trial_count": 1.0,
            "average_pairwise_correlation": 1.0,
            "method": "equicorrelation_effective_independent_trials",
            "fallback_used": False,
            "raw_trial_count": raw_count,
            "correlation_sample_count": len(supplied),
            "unique_behavior_count": len(cleaned),
            "correlation_sample_scope": "supplied_current_train_return_paths",
        }
    correlatable = {
        name: values
        for name, values in cleaned.items()
        if float(np.std(values)) > 1e-15
    }
    if len(correlatable) < 2:
        return {
            ...
            "method": "raw_count_fallback",
            "fallback_used": True,
            "reason": "insufficient_variable_trial_return_paths",
            "correlation_sample_count": len(supplied),
            "unique_behavior_count": len(cleaned),
            "correlatable_behavior_count": len(correlatable),
        }
    length = min(len(values) for values in correlatable.values())
    matrix = np.vstack(
        [values[-length:] for values in correlatable.values()]
    )
    ...
    if not len(finite):
        return {
            ...
            "method": "raw_count_fallback",
            "fallback_used": True,
            "reason": "pairwise_correlation_unavailable",
            "correlation_sample_count": len(supplied),
            "unique_behavior_count": len(cleaned),
        }
    rho = max(0.0, min(0.999999, float(np.mean(finite))))
    unique_count = len(cleaned)
    # The available paths are a representative sample of the current complete
    # Train search. Apply their estimated equicorrelation to the full raw trial
    # history, which also includes prior same-campaign Train trials.
    effective = raw_count / (1.0 + (raw_count - 1.0) * rho)
    return {
        "effective_trial_count": max(1.0, min(float(raw_count), effective)),
        "average_pairwise_correlation": round(rho, 8),
        "method": "equicorrelation_effective_independent_trials",
        "fallback_used": False,
        "raw_trial_count": raw_count,
        "correlation_sample_count": len(supplied),
