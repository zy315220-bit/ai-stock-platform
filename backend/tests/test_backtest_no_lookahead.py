...
class BacktestNoLookaheadTests(unittest.TestCase):
    ...
    def test_signal_uses_history_through_t_and_executes_at_t_plus_1_open(
        ...
        frame = self._frame()
        # Make the next bar's open intentionally extreme. A correct engine may
        # execute there, but calculate_score must never receive that row.
        ...
        def scorer(history: pd.DataFrame):
            seen_last_dates.append(pd.Timestamp(history.index[-1]))
