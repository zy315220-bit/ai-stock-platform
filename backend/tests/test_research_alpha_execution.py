...
def test_mean_reversion_uses_t_signal_and_t_plus_1_execution(
    ...
    # t=60 is oversold below the lower band.  The next bar is execution only.
...
def test_volatility_sizing_uses_current_not_future_natr(
    ...
    download.return_value = raw
    indicators = _indicator_frame(raw)
    # Current signal bar has NATR=4 -> target 2 / 4 = 50% allocation.
    indicators.iloc[60, indicators.columns.get_loc("NATR")] = 4.0
    # Future execution bar is deliberately very low volatility. It must not
    # increase the position chosen from the t=60 signal.
