...
def test_api_exposes_full_gate_without_opening_holdout(monkeypatch) -> None:
    ...
    training = ExperimentResult(
        candidate,
        {
            ...
            "_daily_excess_returns": [0.02] * 300,
            "_daily_strategy_returns": [0.03] * 300,
        },
    ...
    def fake_deflated_sharpe(*args, **kwargs):
        dsr_return_paths.append(args[2])
        return {
            "available": True,
            "multiple_testing_pass": True,
        }

    def fake_pbo(candidate_returns, **kwargs):
        pbo_inputs.append(candidate_returns)
        return {
            "available": True,
            "overfitting_risk_pass": True,
        }

    monkeypatch.setattr(api, "deflated_sharpe_evidence", fake_deflated_sharpe)
    monkeypatch.setattr(api, "cscv_probability_of_backtest_overfitting", fake_pbo)
    monkeypatch.setattr(
        api,
        "hansen_spa_test",
        lambda *args, **kwargs: {
    ...
    assert dsr_return_paths[0]["candidate-1"][0] == 0.03
    assert pbo_inputs[0]["candidate-1"][0] == 0.02
    assert pbo_inputs[1]["candidate-1"][0] == 0.01
    assert payload["model_selection_evidence"]["cscv_pbo_scope"] == (
        "COMPLETE_CURRENT_TRAIN_SEARCH"
    )
    assert payload["model_selection_evidence"][
        "validation_finalist_cscv_pbo_role"
    ] == "DIAGNOSTIC_ONLY"
