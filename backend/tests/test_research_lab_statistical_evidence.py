...
def test_deflated_sharpe_uses_effective_independent_trial_count() -> None:
    ...
    candidate = {"available": True, "period_sharpe_ratio": 0.12, "observations": 500, "skewness": 0.0, "kurtosis": 3.0}
    evidence = deflated_sharpe_evidence(candidate, np.linspace(0.01, 0.08, 100), paths)
    estimator = evidence["effective_trial_estimator"]
    rho = estimator["average_pairwise_correlation"]
    assert evidence["raw_trial_count"] == 100
    assert 1.0 <= evidence["effective_trial_count"] < 12.0
    assert rho > 0.9
    assert evidence["effective_trial_count"] == pytest.approx(
        100 / (1 + 99 * rho),
        abs=1e-6,
    )
    assert estimator["correlation_sample_count"] == 12
    assert estimator["correlation_sample_scope"] == (
        "supplied_current_train_return_paths"
    )


def test_deflated_sharpe_falls_back_to_raw_trials_without_correlation_sample() -> None:
    candidate = {
        "available": True,
    ...
    evidence = deflated_sharpe_evidence(
        candidate,
        np.linspace(0.01, 0.08, 20),
        {"only-path": np.linspace(-0.01, 0.01, 300)},
    )

    assert evidence["effective_trial_count"] == 20
    assert evidence["effective_trial_estimator"]["fallback_used"] is True
    assert evidence["effective_trial_estimator"]["reason"] == (
        "insufficient_trial_return_paths_for_correlation"
    )
